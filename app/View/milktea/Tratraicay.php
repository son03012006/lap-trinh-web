...
<main class="max-w-7xl mx-auto px-6 py-16">
  ...
  <div class="mb-16">
    <h2 class="text-4xl font-extrabold flex items-center gap-3 text-green-700">
      🍊 Trà trái cây
    </h2>
    <div class="w-28 h-1 bg-gradient-to-r from-green-400 to-lime-400 mt-4 rounded-full"></div>
  </div>

  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-14 gap-y-24">
    <?php foreach ($products as $p): ?>
      <div class="text-center group">
        ...
        <h4 class="mt-6 text-lg font-semibold text-green-700 tracking-wide">
  ...
  <?php if (isset($totalPages) && $totalPages > 1): ?>
      <div class="mt-20 flex justify-center items-center gap-2">
          ...
          <?php 
              // Lấy tham số hiện tại để giữ nguyên link khi chuyển trang
              $currentCat = $_GET['cat'] ?? 'traicay'; 
          ...
          <?php if ($currentPage > 1): ?>
              <a href="<?= $baseLink . ($currentPage - 1) ?>" 
                 class="w-10 h-10 flex items-center justify-center rounded-full border border-green-300 text-green-600 hover:bg-green-50 hover:border-green-500 transition">
          ...
          <?php for ($i = 1; $i <= $totalPages; $i++): ?>
              <a href="<?= $baseLink . $i ?>" 
                 class="w-10 h-10 flex items-center justify-center rounded-full font-bold transition
                        <?= $i == $currentPage 
                            ? 'bg-green-600 text-white shadow-lg shadow-green-200 pointer-events-none' 
                            : 'border border-green-300 text-green-600 hover:bg-green-50 hover:border-green-500' ?>">
                  <?= $i ?>
              </a>
          <?php endfor; ?>
...
<div id="productModal"
  ...
  <div class="bg-[#f0fff4] rounded-3xl max-w-4xl w-full p-12
              relative grid grid-cols-1 md:grid-cols-2 gap-12
              shadow-2xl transform transition-all scale-95" id="modalContent">
    ...
    <div class="flex flex-col justify-center">
      <h3 id="modalName"
          class="text-3xl font-extrabold mb-6 text-green-700"></h3>
      ...
      <div class="flex items-baseline gap-2 mb-6">
        ...
        <span id="modalPrice" class="text-3xl font-bold text-lime-600">—</span>
      </div>
      ...
      <div class="mb-6">
          ...
          <textarea id="cartNote" rows="3"
              class="w-full px-4 py-3 rounded-xl
                     border border-lime-300 bg-white
      ...
      <p class="mt-4 italic text-sm text-gray-500 text-center">
        * Trà trái cây ngon nhất khi uống lạnh & ít đá
      </p>
    </div>
        ...
        box.innerHTML += `
          <label class="cursor-pointer group">
            ...
            <div class="px-5 py-2 border-2 rounded-xl text-gray-600 font-bold
                        peer-checked:border-lime-500 peer-checked:bg-lime-50 peer-checked:text-lime-700
                        group-hover:border-lime-300 transition-all">
              Size ${s.size}
            </div>
          </label>
        `;
